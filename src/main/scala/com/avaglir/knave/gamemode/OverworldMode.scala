...
import com.avaglir.knave.{input, Knave}
...
object OverworldMode extends GameMode with Random {
  ...
  override def render(): Unit = {
    ...
    status.drawText(
      ...
      if (inWater) "At sea"
        ...
        Landmass.which(Player.loc / Chunk.DIMENS / 8) match {
          ...
          case None => Landmass.all.minBy { lm =>
              (lm.center * 8 * Chunk.DIMENS - Player.loc).magnitude
            }.print
        }
      },
      Color.WHITE.darker,
    ...
    status.drawText(
      Vector2(2, 3),
      s"Health:   ${stats.health._1.toString.colorize(Color.RED)}/${stats.health._2}",
    )
    ...
    status.drawText(
      Vector2(2, 6),
      s"Accuracy: ${(stats.accuracy * 100).toInt.toString.colorize(c"#3675db")}%",
    )

    status.drawText(Vector2(1, 9), "EQUIPMENT")

    elems.take(5).zipWithIndex.foreach {
      case ((slot, eq), index) =>
        status.drawText(
          Vector2(2, 11 + index),
          s"${slot.name.titleCase.colorize(c"#36db80")}: ${eq.name.titleCase}",
        )
    }
    ...
    ShadowRaycast.calculate(
      ...
      if (tileLoc.componentsClamped(Vector2.UNIT[Int] * Chunk.TILE_DIMENS))
        Overworld.markSeen(tileLoc)
