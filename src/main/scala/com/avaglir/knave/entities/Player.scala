...
object Player extends Entity {
  val repr: RenderTile = RenderTile('@', HSL(Color.YELLOW.hue, 0.67f, 0.7f))
  ...
  message(Equipped.equip(new Weapon(this) {
    ...
    override val weaponClass: WeaponClass = WeaponClass.Axe
    override val baseHit = 0.8f
    override val efficacyFunction: Int => UnitClampedFloat = items.linearEfficacy(0, 100)
    override val basePrice = 30
    override val repr: RenderTile = RenderTile('x')
  }))
