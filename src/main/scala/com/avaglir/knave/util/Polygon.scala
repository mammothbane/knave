...
case class Polygon(points: IntVec*) {
  ...
  private lazy val horiz = points.map {
    _.x
  }

  private lazy val vert = points.map {
    _.y
  }
  ...
  private def precompute(
    ...
    i match {
      ...
      case _ =>
        val k = horiz(i) - (vert(i) * horiz(j)) / (vert(j) - vert(i)) + (vert(i) * horiz(
          i,
        )) / (vert(j) - vert(i))
  ...
  def contains(point: IntVec): Boolean = {
    ...
    def inside(
      ...
      i match {
        ...
        case _
            if vert(i) < point.y && vert(j) >= point.y || vert(j) < point.y && vert(i) >= point.y =>
  ...
  def svgPath: String =
    "M" + this.points.map(point => s"${point.x},${point.y}").mkString(" ") + "z"
...
object Polygon {
  ...
  def fromUnordered(pts: IntVec*): Polygon = {
    ...
    def pbfs[S](init: S, expand: S => List[S]): List[S] = {
      @tailrec
      def search(unexpanded: List[S], seen: List[S]): List[S] =
        unexpanded match {
          case Nil                                => seen
