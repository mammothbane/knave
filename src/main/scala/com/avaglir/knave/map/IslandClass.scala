...
private[map] sealed abstract class IslandClass {
  def rank: Int
  def name: String
...
private [map] object IslandClass {
  object Continent extends IslandClass {
    ...
    override def withModifiers(adj: Option[String], noun: Option[String]): String = "The " + (adj match {
  ...
  object Atoll extends IslandClass {
    ...
    override def withModifiers(adj: Option[String], noun: Option[String]): String = List(adj.getOrElse(""), noun.getOrElse(""), name).filter { _.length > 0 }.mkString(" ")
  }
}
