...
object Nation extends Random {
     ...
     */
    def which(vec: Vector2[Int]) = all.find {
        _.land.exists {
    ...
    lazy val all: Set[Nation] = {
        ...
        val stateClasses = (0 until stateCount).map { _ => poisson(standard.size - 1).clamp(0, 4) + 1 }.sortBy {
            -_
        }
        ...
        lmAssigns.foreach { set =>
            set += sortedLMs.find { elem => !lmAssigns.exists {
                _.contains(elem)
            }
            }.get
        }

        val freeLMs = sortedLMs.filter { elem => !lmAssigns.exists {
            _.contains(elem)
        }
        }
