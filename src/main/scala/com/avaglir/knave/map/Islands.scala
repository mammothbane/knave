...
object Islands {
  ...
  private def boundary(v: IntVec): Boolean =
    World(v, DIMENS) >= threshold && v.adjacent(true).exists { elem =>
      elem.componentsClamped(bounds) && World(elem, DIMENS) < threshold
    }

  lazy val edges: List[Set[IntVec]] = edgesRes(DIMENS)

  def edgesRes(resolution: Int): List[Set[Vector2[Int]]] =
    allRes(resolution).map { island =>
  ...
  def allRes(resolution: Int): List[Set[Vector2[Int]]] = {
    ...
    for {
      ...
      y <- 0 until resolution
    } {
      val vec = Vector2(x, y)
      if (
        chunks(vec) >= threshold && !out.exists {
          _ contains vec
        }
      ) {
        out += bfs(
