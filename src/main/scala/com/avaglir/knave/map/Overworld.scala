...
object Overworld {
  ...
  private var cached: Map[Vector2[Int], Chunk] = surrounding(location).map { loc =>
    (loc, Chunk(loc))
  }.toMap
  ...
  private def cacheExtent: (Vector2[Int], Vector2[Int]) =
    (
      location.clamp(min, max) - Vector2(under, under),
      location.clamp(min, max) + Vector2(over, over),
    )
  ...
  private def surrounding(v: Vector2[Int]): List[Vector2[Int]] = {
    ...
    val out = (chunk.x - under until chunk.x + over).cartesianProduct(
      chunk.y - under until chunk.y + over,
    ).map { case (x, y) => Vector2(x, y) }.toList
  ...
  def setCenter(newCenter: Vector2[Int]): Unit = {
    ...
    val (preserved, load) = surrounding(newCenter).partition {
      cached contains _
    }
    cached =
      (load.map(loc => (loc, Chunk(loc))) ++ preserved.map(loc => (loc, cached(loc)))).toMap
    location = vec
  ...
  private def chunkContaining(location: Vector2[Int]): Option[Chunk] =
   ...
   */
  def markSeen(vecs: Vector2[Int]*): Unit =
    vecs.foreach { vec =>
